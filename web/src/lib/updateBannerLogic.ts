/** Pure logic behind `UpdateBanner.svelte`. */
 ...
 *  `Storage`'s own two methods, injectable for a test with no DOM global. */
...
/** Whether this host has gone long enough without an answer that the banner's
 *  claim to be current is too old to trust. How long is the host's own
 *  `stale_after_days` (`update_state.py` owns the number, and `c64cast
 *  --motd-line` prints from the same one); a response that doesn't say is
 *  never called stale. */
 ...
 *  dismissed. Per-browser, unlike the favorites `api.library()` shares. */
 ...
 *  release it is running — dismissed until the next upgrade rather than for a
 *  fixed spell, since a box with no internet cannot make it stop being true. */
 ...
 *  full quota) just means the banner reappears next load. */
