...
def _field_schema(name: str, type_str: str, *, help: str = "",
                  choices: tuple[str, ...] = (),
                  default: object = None,
                  include_default: bool = True) -> dict[str, Any]:
...
def _section_schema(sd: introspect.SectionDoc) -> dict[str, Any]:
    props = {
        fd.name: _field_schema(fd.name, fd.type, help=fd.help,
                               choices=fd.choices, default=fd.default)
        for fd in sd.fields
    }
    return {
        "type": "object",
    ...
    for od in docs:
        ...
        for p in od.params:
            props[p.name] = _field_schema(
                p.name, p.type, help=p.help, default=p.default,
                include_default=not p.required)
            if p.required:
                required.append(p.name)
        all_of.append({
            "if": {"properties": {"type": {"const": od.name}},
                   "required": ["type"]},
            "then": {
                "properties": props,
                "required": required,
...
def _scenes_schema() -> dict[str, Any]:
    ...
    for fd in field_docs.values():
        if fd.name == "overlays":
            ...
            props[fd.name] = _field_schema(
                fd.name, fd.type, help=fd.help, choices=fd.choices,
                default=fd.default)
    ...
    for sd in introspect.scene_types():
        if not sd.displays:
            continue
        all_of.append({
            "if": {"properties": {"type": {"const": sd.name}},
                   "required": ["type"]},
