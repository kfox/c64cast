...
class VIC:
    ...
    D018_CHAR_DEFAULT: Final = 0x14  # char mode: matrix bank+$0400, charset +$1000
...
# Page A is also the plain (non-flickering) hires layout: every bitmap engage
# in the tree installs it except waveform.py's bank-1 fallback, which needs its
# own matrix offset (waveform.D018_BANK1) to clear the SID payload. The flicker
# handler alternates away from page A and back. Bits 7-4 are the matrix offset
# in units of $0400, bit 3 puts the bitmap at bank+$2000.
