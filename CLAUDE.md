...
- **WLED audio-sync broadcast (bridge Mode 3).** With `[wled] enabled=true`, a process-wide UDP broadcaster ([wled_sync.py](c64cast/wled_sync.py), owned by the `Playlist`, started/stopped around its run loop) synthesizes **WLED "Audio Sync" V2** packets from whichever SID-driven scene is on screen and multicasts them (default **239.0.0.1:11988**), so real WLED LED matrices/strips on the LAN react to the C64's SID with **no mic** on the WLED side (set Sound Sync = "Receive" on the WLED). Source of features is `Scene.features()` — a `MusicModulation` snapshot — which `WaveformScene` (from its scope emulator) and the `generative`+`audio_source="sid"` `SourceScene` (via `SidFeatureStream`) override; every other scene returns None and nothing is sent. Mapping: `level`→`sampleRaw`/`sampleSmth` (0..255); each sounding voice lights the 16-band GEQ bin its note frequency (log-spaced 40 Hz…10 kHz) falls in; `FFT_MajorPeak`=highest active voice. **`samplePeak` (the transient flag most WLED audio effects key off) is derived in the broadcaster** from note-gate rising edges (or `onset`), so WaveformScene — which reports `onset=0` — still flashes on attacks. Packet layout was verified against WLED `usermods/audioreactive/audio_reactive.cpp` (the plan-memory's field list was wrong — the real V2 has `reserved` compiler gaps, not `pressure`/`zeroCrossingCount`). **Pure stdlib UDP, no new dependency/extra.** Modes 1 (control surface) & 2 (DDP/E1.31 pixel sink) and ensemble fan-out are follow-ups. See [[project_wled_bridge]].
