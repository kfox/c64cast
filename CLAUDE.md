...
├── wled_sync.py      WLED bridge Mode 3: broadcast WLED "Audio Sync" UDP
│                     packets synthesized from the active SID scene's
│                     MusicModulation (WledAudioSyncBroadcaster + pure
│                     build_audio_sync_packet); LAN LED matrices react to
│                     the SID. Pure stdlib UDP; [wled] section, Playlist-owned
├── wled_device.py    WLED bridge Mode 1: present c64cast AS a virtual WLED
│                     device — mDNS _wled._tcp (zeroconf) + a subset of the
│                     WLED JSON HTTP/WS API (FastAPI on control_plane's
│                     uvicorn wrapper). WledBridge maps WLED effects↔scenes,
│                     on↔transport + bri↔real output dim, sx/ix↔live
│                     params; one segment per ensemble system.
│                     [wled].listen; `wled` extra; cli-owned
├── wled_sink.py      WLED bridge Mode 2: C64 AS a virtual LED matrix —
│                     receive a realtime pixel stream (DDP :4048 +
│                     WLED realtime UDP :21324, auto-detected) → BGR frame →
│                     the display pipeline quantizes it. WLEDSource
│                     (FrameSource) + WledPixelReceiver + pure parsers;
│                     scene type `wled`; pure stdlib, no extra
...
- **Spatial dither** (`[color].dither`, default `"auto"`; `[color].dither_strength`, default `0.5`) is a separate pre-quantization stage from the `[color]` shaping above, implemented in [c64cast/dither.py](c64cast/dither.py) and threaded into mhires/mcm/hires (not PETSCII) alongside `channel_boost`/`hue_corrections`. `"ordered"` tiles the classic 8×8 Bayer matrix (`dither.bayer_offset`); `"blue_noise"` tiles a 64×64 void-and-cluster mask (`dither.blue_noise_offset`, baked from `scripts/diags/gen_blue_noise.py`'s output — not regenerated at runtime) with the same `strength` scale. Both add their offset to the mode's `flat` pixel array before `quantize_distances`/`quantize_flat` — one vectorized op, position-deterministic (same screen position always gets the same push), so both hold realtime rates with no added frame-to-frame shimmer; skipped when a force-palette remap is active. Blue noise has no periodic low-frequency structure, so it drops Bayer's visible cross-hatch/grid at C64 resolution for the same cost — see `modes._ORDERED_DITHER_OFFSET_FNS` for the dispatch shared by the three call sites (MCM/Hires/MultiHires `compose()`). `"floyd-steinberg"`/`"atkinson"` (`dither.error_diffuse` / `error_diffuse_cells`) are a per-pixel error-diffusion Python loop — not vectorizable, so integrated as a **final-step replacement** rather than a `flat`-level nudge: mhires' `_compose_percell` and MCM's per-cell `fa` block still pick each cell's candidate set (`{bg0,c1,c2,c3}` / `{bg0,bg1,bg2,fg}`) exactly as before through the existing EMA-smoothed histograms, and dithering only replaces the final per-pixel-within-cell code assignment; hires (2-color: global `bg` + per-8×8-cell sampled `fg`) gets the same treatment. `config.resolve_dither_method(dither_setting, scene_type)` resolves `"auto"` per scene type at `build_scene` time (via `_display_mode_for_scene`): static (`slideshow`) → `floyd-steinberg` (composed once, cost is a non-issue); everything else (webcam/video/generative) → `blue_noise` (strictly better than `ordered` at the same realtime/no-shimmer cost — independently-diffused error-diffusion frames would shimmer on motion even though any single frame looks great, and `ordered`'s grid structure is the only thing blue_noise improves on). An explicit non-`"auto"` value (including `"ordered"`, kept for the classic Bayer look) passes through unchanged on any scene — see the FS/Atkinson-on-video caveat in [docs/caveats.md](docs/caveats.md).
...
- **Per-cell color strategy** (`[color].cell_strategy`, default `"auto"`) picks *which* 3 of a cell's present colors fill the mhires `percell` `c1`/`c2`/`c3` slots (no-op on every other mode/palette_mode — MCM picks a single fg per cell by error; the global-4 modes have no per-cell pick). Orthogonal to `palette_mode`, `dither` (per-pixel fill *after* the 3 colors), and `color_match` (distance space). Strategies (`modes._pick_cell_colors`): `frequency` (historical — top 3 by EMA-smoothed population; temporally stable), `luminance` (darkest/median/brightest by `palette.PALETTE_LUMA`, keeps a cell's tonal span), `contrast` (the two luma extremes + the farthest-from-both present color), `error-min` (the trio minimizing summed per-pixel reconstruction error against `{bg0,c1,c2,c3}`; vectorized over `C(6,3)` trios of each cell's top-`ERROR_MIN_POOL_SIZE` present colors — near-optimal, realtime-capable, and provably ≤ frequency's error since the freq top-3 is one of the trios it scores). All keep the **absent-slot → bg0 poison-filler guard** and the sort-by-index delta stability of the frequency path. `config.resolve_cell_strategy(setting, scene_type)` resolves `"auto"` → `error-min` for static scenes (`slideshow` — composed once, best reconstruction for one-time cost) and `frequency` for motion (video/webcam/generative — its stability avoids per-frame slot churn the tonal strategies would introduce re-ranking noisy raw content). Threaded via `_build_display_mode`/`_display_mode_for_scene`; `validate_cell_strategy_cfg` + `--doctor` report the resolved strategy per mhires-percell scene. On typical photographic content the pick rarely diverges (most 4×8 cells hold ≤3 post-quantization colors, so every strategy agrees); busy/high-detail images diverge ~8-12% of pixels for luminance/contrast and ~1-5% for error-min. HW A/B on the U64 (busy slideshow, Cam Link): error-min holds high-detail regions subtly better than frequency with no regression (it ignores rare outlier pixels the reconstruction penalizes); luminance/contrast can add off-color speckle in near-flat regions (they force a tonal extreme onto an outlier), so they stay opt-in and `auto` never selects them.
- **Temporal smoothing / after-image control** (`[color].motion_smoothing`, 0..1, default `0.25`) is one dial over the mhires `percell` path's two *temporal* flicker-suppression buffers (no-op on every other mode/palette_mode): the per-cell color-count EMA (`PERCELL_PICK_EMA_ALPHA`) and the per-pixel/per-cell decision hysteresis (`PERCELL_QUANT/CODE_HYSTERESIS_BONUS`). Both suppress frame-to-frame color churn on noisy video but trade motion-tracking for stability, so on a hard cut an outline from the previous shot **lingers as an after-image** as they decay. `1.0` = full (legacy) smoothing (most stable, ghostiest); `0.0` = no smoothing (`_ema_alpha=1.0`, zero hysteresis → tracks the source frame-exactly, but grain can flicker); intermediate lerps both. An offline stateless-vs-stateful A/B ([scripts/diags/mhires_ema_ghost_ab.py](scripts/diags/mhires_ema_ghost_ab.py)) isolated the causes: the **hysteresis dominates the ghost (~60%)**, the EMA is secondary (~30%), and perceptual `color_match` is NOT a cause (it only recolors the lingering entries more visibly) — which is why one combined dial, not an EMA-only knob, is the control. Threaded `ColorCfg` → `_build_display_mode` → `MultiHiresDisplayMode` (`compose()` reads `self._ema_alpha`); `validate_motion_smoothing_cfg` + `--doctor` bound it 0..1 and note non-default values. The `0.25` default was picked by HW A/B on the U64 (WarGames cuts for ghost, grainy footage for flicker) as the lowest value where flicker stays acceptable — a large ghost reduction vs. the old always-1.0 behavior. Orthogonal to `cell_strategy`/`dither`/`color_match`. See the `motion_smoothing` note in [docs/architecture.md](docs/architecture.md).
- Audio uses 4-bit samples (0-15) written to the SID volume nibble; quality is intentionally low-fi. `[audio].dac_curve` opts into the **Mahoney 8-bit `$D418`** technique (park the SID voices as DC sources + write the full `$D418` byte per sample → ~6-7 effective bits) via a 256-entry amplitude→`$D418` companding table; `"linear"` is the classic 4-bit path, bit-identical. Default is `"auto"` (`config.resolve`/`dac_calibration.resolve_dac_curve_for_backend`): a **per-system calibrated** table if one exists, else the baked `mahoney_ultisid` on the Ultimate (its emulated SID is deterministic across units), else `"linear"` (a physical/unknown SID with no calibration — the baked emulated table wouldn't match it). It shapes the `$D418` DAC only (TeensyROM+ audio + mic/webcam everywhere) — the U64 video default uses the off-bus sampler. Physical 6581/8580 chips (and SID replacements) vary too much chip-to-chip for a baked table, so **`--calibrate-dac`** measures the connected SID's transfer curve via a Cam Link capture on the SID output and writes a table under `calibration/dac/` (gitignored), keyed by a **stable device identity** rather than the connection target — a U64/U2+'s REST `unique_id` (`GET /v1/info`, survives a DHCP re-lease) or a TeensyROM's USB serial number (survives a replug), not the host/IP or OS-assigned device path (`resolve_calibration_key`); `[audio].dac_calibration_profile` overrides this with a user-chosen name, the only way to key a calibration correctly when a roaming TeensyROM+ (no config API, can be moved between physical C64s) is in front of a *different* SID than the one it was calibrated against. On a U64/U2+, every **physical SID socket** reporting a detected chip (`sid_hw_config.detect_sockets`) is measured independently — isolated to `$D400` one at a time, config restored after — since chips vary socket-to-socket too; the file holds one table per socket (schema 2, `"sids": {"1": ..., "2": ...}`) and playback picks the entry matching whichever socket is *currently* live-mapped to `$D400`. `"auto"` then uses the applicable table if one exists, `"calibrated"` requires it. Non-linear curves auto-install the Mahoney SID env (mutually exclusive with `digi_boost`; `"auto"` yields to an explicit `digi_boost` by staying linear). See the `dac_curve` note in [docs/architecture.md](docs/architecture.md), [c64cast/dac_curves.py](c64cast/dac_curves.py) + [c64cast/dac_calibration.py](c64cast/dac_calibration.py). `validate_dac_curve_cfg` (always) plus, per system, exactly one of: the live `_probe_dac_calibration_status` (precise — knows which socket is at `$D400`) when `--doctor` reaches that system, or the offline `_validate_dac_curve_resolution` fallback (approximate — no live device identity, so it hedges via `dac_calibration.list_calibration_files`/`offline_key_is_authoritative` instead of falsely asserting no calibration applies when files exist on disk it can't confirm) under `--skip-probe` or when that system's connectivity probe fails — `validate_load_result` dedupes so a live-reachable system never gets both.
...
- **WLED bridge — two directions, one `[wled]` section.** Each direction is one combined string field: `disabled` (default) | `enabled` | `[host][:port]` — the value encodes on/off **and** endpoint (`config.parse_wled_endpoint` / `resolve_wled_broadcast` / `resolve_wled_listen`; bare `HOST` overrides host, leading `:PORT` overrides port).
  - **Mode 3 — `broadcast` (audio-sync out).** A process-wide UDP broadcaster ([wled_sync.py](c64cast/wled_sync.py), owned by the `Playlist`, started/stopped around its run loop) synthesizes **WLED "Audio Sync" V2** packets from whichever SID-driven scene is on screen and multicasts them (`enabled` → **239.0.0.1:11988**), so real WLED LED matrices/strips on the LAN react to the C64's SID with **no mic** on the WLED side (set Sound Sync = "Receive" on the WLED). Source of features is `Scene.features()` — a `MusicModulation` snapshot — which `WaveformScene` (from its scope emulator) and the `generative`+`audio_source="sid"` `SourceScene` (via `SidFeatureStream`) override; every other scene returns None and nothing is sent. Mapping: `level`→`sampleRaw`/`sampleSmth` (0..255); each sounding voice lights the 16-band GEQ bin its note frequency (log-spaced 40 Hz…10 kHz) falls in; `FFT_MajorPeak`=highest active voice. **`samplePeak`** is derived in the broadcaster from note-gate rising edges (or `onset`), so WaveformScene — which reports `onset=0` — still flashes on attacks. Packet layout verified against WLED `usermods/audioreactive/audio_reactive.cpp` (real V2 has `reserved` compiler gaps, not `pressure`/`zeroCrossingCount`). **Pure stdlib UDP, no extra.**
  - **Mode 1 — `listen` (control surface in).** c64cast presents itself **as** a virtual WLED device ([wled_device.py](c64cast/wled_device.py), cli-owned, started/stopped alongside the control plane): it advertises `_wled._tcp` over mDNS (`zeroconf`) and serves a subset of the WLED **JSON HTTP + WS API** (`/json`, `/json/state|info|si|eff|pal`, POST `/json[/state]`, WS `/ws`) on a FastAPI app running on `control_plane.ControlServer`'s uvicorn wrapper, so the **WLED app / python-wled / Home Assistant** can discover and control it — no c64cast client. `WledBridge` maps WLED **effects↔scenes** (`seg[].fx` → `Playlist.request_jump`), **on↔transport, bri↔real output dim (independent axes)** (`on=false` → pause, `on=true` → resume — the *only* transport control; `bri` is a real screen dim decoupled from transport — the effective brightness `(master bri/255)*(seg bri/255)` is pushed onto the playlist + live display mode as `DisplayMode.user_dim`, which folds multiplicatively into the fade LUT so the picture visibly darkens, `bri=0` dims fully to black but does **not** pause; `Playlist.user_dim` is re-stamped onto each fresh scene's mode in `_safe_setup` so a dim survives auto-advance. Brightness never touches transport — the earlier `bri=0 → pause` reset the machine to BASIC mid-drag, the "flashing cursor" HW bug), **sx/ix sliders↔the current scene's `LIVE_PARAMS`** (same seam midi_control sweeps; `_IX_TARGETS` = `source.scale`/`source.intensity`/`effect.intensity`/`scene.gain`, so generators, the pulse/rgb_shift effect intensity, and the SID scope scenes' trace `gain` all answer ix — the `scene.` prefix reaches a scope scene, which *is* the renderer with no source/effect holder), **palette dropdown↔`[color].palette_mode`** (`seg[].pal` → the current scene's live `DisplayMode.set_palette_mode`, also clearing any color force), and **color picker↔forced palette** (`seg[].col`'s up-to-3 slots → nearest C64 colors → `palette.build_fixed_color_map` + the mode's force toggle, so the scene posterizes live to the picked colors on mcm/mhires; echo-only on modes without the setters). Ensemble = **one WLED segment per system** (segment *i* ↔ the *i*-th system; top-level on/bri hit all, per-segment fields hit that one; shared effect list from system[0]). `enabled` → binds `0.0.0.0:8080` (mDNS SRV carries the real port for app discovery). Needs the **`wled` extra** (zeroconf + fastapi + uvicorn). The `[wled].listen` on the first system's config governs the cluster device. The self-served `GET /` control page mirrors exactly these functional controls (power, a single **master** brightness slider driving top-level `bri` — the same field the WLED app's own brightness slider uses, so the two stay in sync — per-system scene/palette selects, per-segment speed/intensity sliders, color picker; each segment titled by `seg[].n` = the system name, single-system → the configurable `[wled].name`). A scene pick blurs the `<select>` and schedules a short refresh burst so the capability hints don't freeze behind the "skip refresh while a control is focused" guard. Two HW-surfaced robustness details: `pal`/`col` map onto the same C64 palette so each is applied only when it **changed** from the last echo (the WLED app re-POSTs the full segment every time, so an unchanged `col` would otherwise clobber a palette pick); and the `/ws` handler **proactively pushes** state on a `_WS_PUSH_INTERVAL_S` timeout when it changed on its own (playlist auto-advance / a queued jump landing), so the app's Scene field doesn't go stale. **Capability hints on `/`:** each seg dict carries a `c64` vendor key (`_seg_caps`) of per-control booleans (`pal`/`col`/`sx`/`ix`) mirroring the write-path applicability guards — `pal` ⇐ the mode has `set_palette_mode`, `col` ⇐ also `set_color_map`, `sx`/`ix` ⇐ `_resolve_live_target` finds a matching `LIVE_PARAM`; the self-served page grays out (disables + dims + tooltips) the dead palette/color/speed/intensity controls (Scene/Power/Brightness never gated). The key rides the `/json` poll + the WS push so hints refresh on auto-advance; WLED clients ignore the unknown seg key. **Remaining limitation:** the third-party WLED *app* renders a fixed control set c64cast can't remotely disable, so a palette/color pick there on a scene that can't use it (blank/char modes) is still a silent no-op — only our own `/` page reflects capability.
  - **Mode 2 — pixel sink (`wled` scene type).** The C64 **as** a virtual WLED LED matrix: a `wled` scene ([wled_sink.py](c64cast/wled_sink.py)) receives a realtime pixel stream over UDP and renders it to the C64. Two protocols, auto-detected on their standard ports (both bound at once): **DDP** (4048, multi-packet frames + push flag — what LedFx/xLights emit) and **WLED realtime UDP** (21324; WARLS/DRGB/DRGBW/DNRGB). A `WLEDSource(BaseFrameSource)` publishes the assembled BGR frame into a `SourceScene`, so the ordinary display pipeline (palette/dither/color_match) quantizes it exactly like a webcam/generator — no audio, no SID. Per-scene `sink_width`/`sink_height` (default 320×200, 1..1024) declare the virtual matrix the sender targets; `display` defaults to `mhires`. A bind-port conflict self-aborts the scene (playlist advances). **Pure stdlib UDP, no extra.** The WLED phone app is a *controller* and can't send pixels — drive it with a real sender (LedFx/xLights) or [scripts/diags/wled_pixel_sender.py](scripts/diags/wled_pixel_sender.py). **Follow-ups:** E1.31/sACN (multi-universe; LedFx/xLights already speak DDP) and ensemble-as-one-tiled-matrix. See [[project_wled_bridge]].
