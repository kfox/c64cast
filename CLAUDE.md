...
├── asid.py           Pure ASID-protocol codec: decode() = ASID MIDI SysEx
│                     (packed SID register writes) → AsidUpdate; encode_*() =
│                     the inverse pack side (host/sender). No mido/hardware deps;
│                     decodes the 0x30 timing recipe + 0x31 buffering bit
├── asid_broadcast.py AsidBroadcaster: c64cast as an ASID *host* — pack a
│                     per-frame $D4xx image (delta-encoded) out a MIDI output
│                     port to drive external ASID clients in sync. Wired into
│                     WaveformScene ([scene].asid_broadcast_port); Midi/Asid TBD
...
- **ASID host/broadcast (c64cast as ASID *sender*).** The inverse of the ASID client: emit ASID MIDI SysEx so external ASID clients (TherapSID, SidStation, USBSID-Pico, DeepSID, another c64cast) play in sync with c64cast's own SID + HDMI scope. The encoder (`encode_registers` + `encode_start`/`stop`/`chars`/`speed`/`sid_type`) lives in [asid.py](c64cast/asid.py) as the pure inverse of `decode()` (`decode(encode_registers(regs)) == regs`); [asid_broadcast.py](c64cast/asid_broadcast.py) `AsidBroadcaster` owns a mido **output** port + per-chip **delta** encoding (only changed regs, via the pure `frame_messages` helper) + hard-restart `control_first` synthesis. Wired into `WaveformScene` via `[scene].asid_broadcast_port` (needs the `midi` extra): it packs each PLAY tick's `$D4xx` image (from the host emu it already runs for the scope) and ships one ASID frame per tick — multi-SID (0x50-0x5F) falls out of the per-chip banks. Best-effort (a port error disables broadcast, never crashes the scene). `MidiScene`/`AsidScene` broadcast are follow-ups — [[project_asid_host_broadcast]], [[project_asid_host_midi_asid_scenes]].
