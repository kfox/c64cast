...
- **WLED bridge — two directions, one `[wled]` section.** Each direction is one combined string field: `disabled` (default) | `enabled` | `[host][:port]` — the value encodes on/off **and** endpoint (`config.parse_wled_endpoint` / `resolve_wled_broadcast` / `resolve_wled_listen`; bare `HOST` overrides host, leading `:PORT` overrides port).
  ...
  - **Mode 1 — `listen` (control surface in).** c64cast presents itself **as** a virtual WLED device ([wled_device.py](c64cast/wled_device.py), cli-owned, started/stopped alongside the control plane): it advertises `_wled._tcp` over mDNS (`zeroconf`) and serves a subset of the WLED **JSON HTTP + WS API** (`/json`, `/json/state|info|si|eff|pal`, POST `/json[/state]`, WS `/ws`) on a FastAPI app running on `control_plane.ControlServer`'s uvicorn wrapper, so the **WLED app / python-wled / Home Assistant** can discover and control it — no c64cast client. `WledBridge` maps WLED **effects↔scenes** (`seg[].fx` → `Playlist.request_jump`), **on/bri↔transport** (`on=false`/`bri=0` → pause; brightness echoed, live pixel-dim is a follow-up), and **sx/ix sliders↔the current scene's `LIVE_PARAMS`** (same seam midi_control sweeps). Ensemble = **one WLED segment per system** (segment *i* ↔ the *i*-th system; top-level on/bri hit all, per-segment fields hit that one; shared effect list from system[0]). `enabled` → binds `0.0.0.0:8080` (mDNS SRV carries the real port for app discovery). Needs the **`wled` extra** (zeroconf + fastapi + uvicorn). The `[wled].listen` on the first system's config governs the cluster device.
