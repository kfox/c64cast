...
**Audio is on by default** (`AudioCfg.enabled` defaults True); `--no-audio` mutes. On the U64, video audio defaults to the high-fidelity **Ultimate Audio FPGA PCM sampler** (`[audio].backend = "auto"` → sampler when available; see [sampler.py](c64cast/sampler.py)); `backend = "dac"` forces the lo-fi 4-bit `$D418` DAC (the only path on TeensyROM, and the path for mic/webcam audio everywhere). If sampler audio drifts against the (host-clock-paced) video over minutes, the unit's real sampler clock differs from the firmware-nominal 6.25 MHz: calibrate `[audio].sampler_clock_hz` with [scripts/diags/sampler_clock_calib.py](scripts/diags/sampler_clock_calib.py) (a starting estimate; final tuning by ear against video — ear-tuned to 6155000 on one U64-II where the nominal ran ~2% slow; likely a firmware clock case to confirm with Gideon). Flag groups (`-h` shows them grouped): `connection`, `quick playback`, `video input`, `audio`, `vision input`, `playlist`, `introspection`, `debug`.
...
│                     drives [audio].dac_curve (linear | mahoney_ultisid)
...
- Audio uses 4-bit samples (0-15) written to the SID volume nibble; quality is intentionally low-fi. `[audio].dac_curve = "mahoney_ultisid"` opts into the **Mahoney 8-bit `$D418`** technique (park the SID voices as DC sources + write the full `$D418` byte per sample → ~6-7 effective bits) via a baked emulated-UltiSID companding table; `"linear"` (default) is the classic 4-bit path, bit-identical. It shapes the `$D418` DAC only (TeensyROM+ audio + mic/webcam everywhere) — the U64 video default uses the off-bus sampler. See the `dac_curve` note in [docs/architecture.md](docs/architecture.md) and [c64cast/dac_curves.py](c64cast/dac_curves.py); physical (non-emulated) SIDs vary too much for a baked table and need per-unit calibration (a follow-up).
