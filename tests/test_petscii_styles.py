...
class StyleComposeShapeTest(unittest.TestCase):
    ...
    def test_every_style_returns_correctly_shaped_buffers(self):
        ...
        for name in ps.STYLE_NAMES:
            ...
            self.assertTrue((color & 0x0F == color).all(),
                            f"{name}: color RAM has high-nibble bits set")
...
class IndividualStyleBehaviorTest(unittest.TestCase):
    ...
    def test_color_only_paints_every_cell_full_block(self):
        ...
        self.assertTrue((screen == 0xA0).all(),
                        "color_only must fill every cell with SC_FULL_BLOCK")
    ...
    def test_inverse_pop_screen_is_only_space_or_block(self):
        ...
        self.assertLessEqual(unique, {0x20, 0xA0},
                             f"inverse_pop screen has unexpected codes {unique}")
        ...
        for c in np.unique(color):
            self.assertIn(int(c), pop,
                          f"inverse_pop produced non-pop FG {int(c)}")
    ...
    def test_neon_color_avoids_gray_axis(self):
        ...
        for c in np.unique(color):
            self.assertNotIn(int(c), GRAY_INDICES,
                             f"neon picked gray-axis color {int(c)}")
    ...
    def test_random_glyph_is_stable_per_cell(self):
        ...
        np.testing.assert_array_equal(s1, s2,
            "random_glyph cell→glyph mapping must be stable across frames")

    def test_letter_rain_uses_only_a_to_z(self):
        screen, _ = ps.LetterRainStyle().compose(_frame(), _BOOST, _HUE)
        self.assertTrue((screen >= 0x01).all() and (screen <= 0x1A).all(),
    ...
    def test_hue_corrections_reach_styles_and_rescue_purple(self):
        ...
        self.assertNotIn(4, {int(c) for c in np.unique(plain)},
                         "expected NO purple without the hue band")
        self.assertIn(4, {int(c) for c in np.unique(rescued)},
...
class CyclingTest(unittest.TestCase):
    ...
    def test_petscii_random_resolves_at_construction(self):
        ...
        self.assertIn(m.style, ps.STYLE_NAMES,
                      "'random' must resolve to a concrete style")
