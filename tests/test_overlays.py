...
class RegistryTest(unittest.TestCase):
    ...
    def test_known_overlays(self):
        ...
        for expected in ("scrolling_text", "spectrum_petscii",
                         "clock", "weather"):
...
class ScreenCodeTest(unittest.TestCase):
    ...
    def test_digits_pass_through(self):
        self.assertEqual(ascii_to_screen("0123"),
    ...
    def test_renders_screen_row_after_setup(self):
        ...
        ov = ScrollingTextOverlay(
            messages=[{"text": "HI", "color": "yellow",
                       "pre_delay_s": 0.0, "pause_time_s": 0.0,
                       "style": "static"}],
            row=10, speed_cells_per_s=4.0)
...
class SpectrumPetsciiTest(unittest.TestCase):
    ...
    def test_zero_samples_produce_no_bars(self):
        ...
        ov = PetsciiSpectrumOverlay(audio=audio, placement="bottom",
                                    height_rows=8)
        buffers = _make_buffers()
        ...
        self.assertTrue((buffers["screen"][top:bot] == SC_SPACE).all(),
                        "silence shouldn't light any cells")
    ...
    def test_loud_tone_lights_bars(self):
        ...
        ov = PetsciiSpectrumOverlay(audio=audio, placement="bottom",
                                    height_rows=12, gain=2.0)
        buffers = _make_buffers()
        ov.compose(buffers, scene=MagicMock(), t=0.0)
        self.assertTrue((buffers["screen"] == SC_FULL).any(),
                        "a loud tone should light at least one bar")
...
class ClockTest(unittest.TestCase):
    ...
    def test_renders_time_string_to_top_right(self):
        ...
        ov = ClockOverlay(corner="top-right", format="%H:%M",
        ...
        self.assertEqual(bytes(buffers["screen"][35:40]),
                         bytes([0x31, 0x32, 0x3A, 0x33, 0x34]))
...
class WeatherTest(unittest.TestCase):
    ...
    def test_uses_cache_when_fetch_fails(self):
        ...
        with patch("c64cast.overlays.weather._fetch_open_meteo",
                   side_effect=Exception("boom")):
            ov = WeatherOverlay(provider="open-meteo", lat=0.0, lon=0.0,
                                refresh_minutes=10)
            api = _fake_api()
            ...
            with self.assertLogs("c64cast.overlays.weather",
                                 level="ERROR") as cap:
...
class CallsignTest(unittest.TestCase):
    ...
    def test_renders_text(self):
        ...
        self.assertEqual(bytes(buffers["screen"][base:base + 5]),
                         bytes([0x17, 0x35, 0x01, 0x02, 0x03]))
...
class CountdownTest(unittest.TestCase):
    ...
    def test_bad_format_falls_back(self):
        ...
        with self.assertLogs("c64cast.overlays.countdown",
...
class LogoTest(unittest.TestCase):
    ...
    def test_renders_file_at_corner(self):
        ...
        try:
            ...
            self.assertEqual(bytes(buffers["screen"][0:2]),
                             bytes([0x01, 0x02]))   # AB
            self.assertEqual(bytes(buffers["screen"][40:42]),
                             bytes([0x03, 0x04]))   # CD
        finally:
            os.unlink(path)
    ...
    def test_missing_file_uses_placeholder(self):
        ...
        from c64cast.overlays.logo import LogoOverlay
        with self.assertLogs("c64cast.overlays.logo",
                             level="WARNING") as cap:
...
class NetworkCollectTest(unittest.TestCase):
    ...
    def test_collect_gathers_each_item(self):
        ...
        ov._target_port = 80
        with patch.object(net, "_outbound_ip", return_value="192.168.1.2"), \
    ...
    def test_collect_preserves_previous_value_on_failure(self):
        ...
        with patch.object(net, "_outbound_ip",
...
class OBSStatusTest(unittest.TestCase):
    ...
    def test_poll_once_formats_scene_and_dropped(self):
        ...
        ov._connect = lambda: _FakeOBSClient(scene="Main", skipped=2,
                                             render_skipped=3)
        lines = ov._poll_once()
