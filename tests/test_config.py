...
class FormatTomlErrorTest(unittest.TestCase):
    ...
    def test_uses_error_attrs_when_present(self):
        err = type("E", (), {"lineno": 2, "colno": 5, "msg": "bad value",
                             "doc": "a = 1\nb = ?\n"})()
...
class MergeCLITest(unittest.TestCase):
    ...
    def test_cli_value_overrides_config_value(self):
        ...
        merged = cfgmod.merge_cli(
            cfg, self._make_args(url="http://from-cli", audio=True))
    ...
    def test_cli_can_override_nested_audio_fields(self):
        ...
        merged = cfgmod.merge_cli(
            cfg,
            self._make_args(audio_device=3, sample_rate=22050,
                            mic_sensitivity=2.0, noise_gate=0.1),
        )
...
class ValidateSceneCfgTest(unittest.TestCase):
    ...
    def test_commercial_scene_falls_back_to_default_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(os.path.join(tmp, "assets", "videos", "ok.mp4"),
                      "w") as f:
                f.write("")
            os.chdir(tmp)
            try:
                s = cfgmod.SceneCfg(type="commercial")
                cfgmod.validate_scene_cfg(s, self._cfg(),
                                          audio_enabled=False)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                with self.assertRaisesRegex(
                        ValueError,
                        "default directory .* is missing or empty"):
                    cfgmod.validate_scene_cfg(s, self._cfg(),
                                              audio_enabled=False)
            finally:
                os.chdir(cwd)
    ...
    def test_waveform_scene_falls_back_to_default_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(os.path.join(tmp, "assets", "sids", "tune.sid"),
                      "w") as f:
                f.write("")
            os.chdir(tmp)
            try:
    ...
    def test_slideshow_scene_falls_back_to_default_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(os.path.join(tmp, "assets", "pictures", "p.jpg"),
                      "w") as f:
                f.write("")
            os.chdir(tmp)
            try:
    ...
    def test_slideshow_image_duration_s_must_be_positive(self):
        s = cfgmod.SceneCfg(type="slideshow", file="pic.jpg",
                            image_duration_s=0.0)
        with self.assertRaisesRegex(ValueError,
                                    "image_duration_s must be > 0"):
            cfgmod.validate_scene_cfg(s, self._cfg(), audio_enabled=False)
    ...
    def test_slideshow_display_hires_edges_substituted_with_mhires(self):
        ...
        # for stills).
        self.assertEqual(
            cfgmod._resolve_slideshow_display("hires_edges"), "mhires")
        # Other explicit choices pass through.
        for name in ("hires", "mhires", "mcm", "petscii"):
            self.assertEqual(
                cfgmod._resolve_slideshow_display(name), name)
    ...
    def test_overlay_requires_audio_with_audio_enabled_passes(self):
        ...
        s = cfgmod.SceneCfg(
            type="webcam", display="petscii",
            overlays=[{"type": "spectrum_petscii"}])
    ...
    def test_launcher_scene_falls_back_to_default_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(os.path.join(tmp, "assets", "programs", "g.prg"),
                      "wb") as f:
                f.write(b"\x01\x08")
            os.chdir(tmp)
            try:
    ...
    def test_launcher_scene_valid_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            s = cfgmod.SceneCfg(type="launcher", file=self._prg(tmp),
                                duration_s=90.0, input_source="cia")
            cfgmod.validate_scene_cfg(s, self._cfg(), audio_enabled=False)
...
class ResolveFileSpecTest(unittest.TestCase):
    ...
    def test_directory_with_no_matches_raises(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError,
                                        "contains no files with extension"):
    ...
    def test_glob_expansion(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            got = cfgmod.resolve_file_spec(os.path.join(tmp, "*.sid"),
                                           self.EXTS, label="waveform")
            self.assertEqual([os.path.basename(p) for p in got],
                             ["alpha.sid", "beta.sid"])
    ...
    def test_glob_with_no_matches_raises(self):
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(ValueError, "matched no files"):
                cfgmod.resolve_file_spec(os.path.join(tmp, "nope-*.sid"),
                                         self.EXTS, label="waveform")
    ...
    def test_comma_combination_unions_and_dedupes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            spec = (f"{literal}, {tmp}, "          # x.sid + dir (x,y,z)
                    f"{os.path.join(tmp, 'z.sid')}")  # dup
    ...
    def test_whitespace_only_entries_ignored(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                cfgmod.resolve_file_spec(f"{p}, , ", self.EXTS,
                                         label="waveform"),
                [p])
    ...
    def test_commercial_scene_rejects_dir_with_no_videos(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError,
                                        "contains no files with extension"):
                cfgmod.validate_scene_cfg(s, cfgmod.Config(),
                                          audio_enabled=False)
...
class FollowerOnlyRotationFilterTest(unittest.TestCase):
    ...
    def test_follower_only_excluded_from_rotation(self):
        self.cfg.scenes = [
            ...
            cfgmod.SceneCfg(type="blank", name="hello",
                            follower_only=True),
        ]
    ...
    def test_follower_only_still_validated(self):
        ...
        self.cfg.scenes = [
            ...
            cfgmod.SceneCfg(type="blank", name="hello",
                            follower_only=True,
                            display="hires"),  # invalid for blank scene
        ]
        with self.assertRaises(ValueError):
