...
class RenderTest(unittest.TestCase):
    ...
    def test_blank_compose_writes_strip_via_page_flip(self):
        ...
        for t_step in (0.2, 0.4, 0.6, 1.0, 2.0):
            ov.compose(_make_buffers(), self._scene(BlankDisplayMode()), t_step)
        # Page-flip writes target the strip area of each page. Strip starts
        # at row 8 of a 25-row screen → cell offset 320 → $page+$140.
        strip_calls = [
            c for c in api.write_memory_file.call_args_list if c.args[0] in ("0540", "0D40")
        ]
        self.assertTrue(strip_calls, "expected at least one strip page-flip write")
        # Cell-expansion only writes SC=$A0 ("on") or SC=$20 ("off") —
        # no other chars.
        for call in strip_calls:
            data = call.args[1]
            self.assertTrue(
                all(b in (0x20, 0xA0) for b in data),
        ...
        self.assertTrue(
            any(0xA0 in c.args[1] for c in strip_calls),
            "lit cells must use SC=$A0 once text is on-screen",
        )

    def test_mcm_compose_paints_with_multicolor_flag(self):
        ov = _make_overlay(messages=[{"text": "HI", "color": "white"}], speed_cells_per_s=80.0)
    ...
    def test_animation_advances_over_time(self):
        ...
        def drive(n_frames, t_start):
            for i in range(n_frames):
                ov.compose(_make_buffers(), self._scene(BlankDisplayMode()), t_start + i * 0.02)
        ...
        self.assertNotEqual(
            bytes_t1, bytes_t2, "strip bytes at different t should differ as text scrolls"
        )
...
class SmoothScrollTest(unittest.TestCase):
    ...
    def test_shadow_d016_written_each_sub_cell_step(self):
        api = MagicMock()
        ov = _make_overlay(messages=[{"text": "X", "color": "white"}], speed_cells_per_s=8.0)
        ov.setup(api=api, scene=self._scene(BlankDisplayMode()))
    ...
    def test_raster_irq_installed_on_setup(self):
        ...
        handler_uploads = [
            c for c in api.write_memory_file.call_args_list if c.args[0].lower() == "c000"
        ]
        self.assertTrue(handler_uploads, "expected the raster IRQ handler to be uploaded to $C000")
        # IRQ vector at $0314 swung to $C000.
        vector_writes = [c for c in api.write_regs.call_args_list if c.args[0] == "0314"]
        self.assertTrue(vector_writes, "expected the IRQ vector $0314/$0315 to be hooked")
    ...
    def test_raster_irq_uninstalled_on_teardown(self):
        ...
        vector_writes = [c for c in api.write_regs.call_args_list if c.args[0] == "0314"]
        self.assertTrue(vector_writes, "expected teardown to restore the IRQ vector $0314")
        last_vector = vector_writes[-1].args
...
class IsBusyTest(unittest.TestCase):
    ...
    def test_busy_until_messages_exhausted(self):
        ...
        self.assertFalse(ov.is_busy(), "should not be busy after all messages scrolled off")
...
class ColorCycleTest(unittest.TestCase):
    ...
    def test_cycle_wraps_back_to_config(self):
        ...
        self.assertEqual(last, "config", "full cycle should land back at 'config'")
...
class FollowerComposeTest(unittest.TestCase):
    ...
    def test_follower_uses_published_solid_color(self):
        ...
        self.assertIn(yellow, unique, "follower must paint the conductor's published color")
        self.assertNotIn(white, unique, "follower must NOT use its local message color")
...
class LoopTest(unittest.TestCase):
    ...
    def test_loop_cycles_back_to_first_message(self):
        ...
        self.assertIn(0, seen_indices[first_one:], "loop=True must wrap _msg_idx back to 0")
