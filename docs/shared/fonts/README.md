...
**These files are not covered by c64cast's MIT license.** Both are licensed
...
| Font | Version | Copyright | Upstream | License |
|---|---|---|---|---|
...
license and license URL in its own `name` table as well.

## If you touch anything in here

- **Keep the `OFL-*.txt` files with the fonts.** The license requires the
  copyright notice and license text to accompany every copy. Anything that
...
- **`pre-commit` skips this directory** (see the `exclude` in
  ...
  whitespace fixers happily reformat a license file, which is not ours to
  reformat.
