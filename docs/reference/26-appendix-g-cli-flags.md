...
| **`--save-settings`** | Persist this invocation's machine-relevant flags (`-u/--url`, `-d/--device`, `-D/--audio-device`, `--sid-model`, `-s/--system`) into the machine-settings file ($C64CAST_SETTINGS, else ~/.config/c64cast/settings.toml), then exit. Merges with any existing file; secrets are never written. |
...
| Flag | Description |
|---|---|
| **`-v`, `--verbose`** | Increase log verbosity (default: INFO; -v enables DEBUG, -vv adds HTTP transport and the web console's server log, -vvv adds the background polls' own reads and the console's access log) |
