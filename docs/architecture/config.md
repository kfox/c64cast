...
**The URL fallback is version-pinned.** When no local path works, `config_serialize.DEFAULT_SCHEMA_PATH` is the raw-GitHub URL — built at import time by `_published_schema_url(__version__)`, so it names the tag (`v0.1.0`) of the release doing the writing rather than `main`. The config being generated is one *this* version accepts, and the schema on `main` moves: a key added since, a `choices` list that has grown. Validating against it reports errors that aren't errors and misses ones that are. A source checkout has no tag to name — `__version__` is the `"0+unknown"` `importlib.metadata` fallback, and a dev tree is ahead of every tag regardless — so the function falls back to `main` for anything not starting with a digit, which is the correct answer there.
