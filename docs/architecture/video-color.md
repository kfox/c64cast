...
**One teardown step here is not an independent promise.** `uninstall_bank_swap_irq` hands its six writes to `run_teardown_steps` so a link hiccup on any one cannot starve the rest ([config.md](config.md#_teardownpy--a-teardowns-steps-are-independent-guarantees)) — but the last of them, re-enabling CIA #1 Timer A, is only safe once `$0314/$0315` is back at `$EA31`. With the vector still on the `$C500` handler, re-arming the jiffy IRQ hands every IRQ to it: `$D019`'s raster flag latches regardless of `$D01A`, so the handler sees the raster bit and re-flips `$DD00` to bank 2 on the next frame — undoing the `VIC bank 0` step that ran just before it — and once the next scene writes over `$C500` it jumps into whatever is there. Leaving Timer A masked costs the kernal keyboard scan and the C= / CTRL / SHIFT poller for the rest of the session, which is the lesser failure, so the unmask step reads the vector step's outcome and logs its skip rather than firing unconditionally. `tests/test_reu_video.py`'s `BankSwapIrqTeardownGuardTest` pins both directions.
