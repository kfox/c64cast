...
`ERROR_MIN_HYSTERESIS_MARGIN` (`base.py`, 0.25 — the same value as `BG0_HYSTERESIS_MARGIN`) fixes this the same way bg0 is kept sticky: `pick_cell_colors`/`_pick_cell_colors_error_min` take the previous frame's trio and keep it unless a challenger's summed error is at least that fraction lower, so a genuine near-tie stops flip-flopping while a real content change (whose error improvement is overwhelming, not a near-tie) still wins on a single frame. Scaled by `motion_smoothing` (`self._error_min_margin`, derived alongside `_ema_alpha`/`_quant_hysteresis`/`_code_hysteresis` in the setter) so `motion_smoothing = 0` disables it along with the mode's other temporal smoothing, consistent with that knob's "0.0 = none, tracks the source exactly" contract.
