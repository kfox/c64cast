...
**Flicker defers twice as far.** Its commit is additionally gated on phase 0, so a rejected commit waits for the next phase-0 field: worst case 2 fields = 33.4 ms against a ~38.5 ms host frame period at 26 fps. That is also the likely reason flicker tore ~4.4× more often than plain before the gate — half as many commit opportunities per second, so a halt is likelier to have covered all of them. That attribution is inferred from the handler shape, not measured; if the gate ever underperforms, the phase-0 gate is the next thing to examine.
