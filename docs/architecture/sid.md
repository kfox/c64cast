...
Nothing in the audio path changed for any of this. The player already `JSR`s the tune's own play routine, so `STA $D420` reaches the bus and whatever sits there answers; `_decode_extra_sid_addr` already accepts the full PSID `$D000-$DFF0` window, including the `$DE00`/`$DF00` cartridge-I/O one (which collides with a TR+ or U2+ occupying the port — a hardware conflict, documented in caveats.md, not something code can resolve). The declaration affects what gets *said*, and — via `host_chip_fit` below — which tune a pool picks. It never changes a chip.
