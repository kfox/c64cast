...
**Every INIT/PLAY call is bounded by steps and wall clock, not only by emulated cycles.** py65 binds all 105 undocumented opcodes to `inst_not_implemented` with a `cycletime` of 0, so a routine built out of them advances `processorCycles` not at all and a cycle cap alone cannot terminate it — a crafted PLAY measured 7-21 s per `tick_play()` against a cap the comments budget at ~4 ms. `_run_routine` therefore also counts interpreter steps (a step budget equal to the cycle cap can never bind on a routine the cycle cap would have caught first, since every implemented opcode costs at least 2 cycles), and it **refuses an undocumented opcode outright** with a warning rather than executing it: py65 advances the PC by 2 regardless of the real instruction's length, which derails the instruction stream and yields both a wrong `$D4xx` shadow and a wrong write footprint — and that footprint is what `api._find_free_layout` places the C64-side player from. Failing the pass loudly beats rendering a trace derived from a program the C64 will not run. `cap` is an explicit argument per caller rather than inferred from the log tag, and both budget tests are re-guarded by the sentinel so a routine that completes on the very step that crosses a budget is not reported as capped.

Per-call soundness still leaves the aggregate open, so `ram_write_footprint` / `ram_play_access_footprint` stop at `FOOTPRINT_DEADLINE_S` (2 s) and return the partial bitmap with a warning — both are already documented as a sample rather than a proof — and the SHIFT-cycle candidate walk is capped at `_MAX_CYCLE_CANDIDATES`, the way `setup()`'s scan has always been capped at `_UNIFIED_LAYOUT_MAX_SONGS`. `num_songs` is a raw 16-bit header field nothing else bounds, and a tune declaring 65535 subtunes whose PLAY blocks every VIC bank turned one SHIFT press into hours of frozen show with the audio already silenced.

**Extra-SID addresses are validated where they are decoded.** `_decode_extra_sid_addr` accepts only what the PSID spec permits: even bytes resolving into `$D420-$D7E0` or `$DE00-$DFE0`. Anything else — an odd byte, a byte below `$42`, a base equal to `$D400` — is refused and the tune degrades to single-SID, which is the fallback the code always claimed. `detect_sid_addresses` additionally appends a base only when its 25-byte window touches none already accepted, because `TrappedRam._addr_map` is a dict comprehension over the bases and a later bank silently wins every colliding key: two chips at one address (reachable from the header, and from an HVSC `_NSID` filename over a header that already claims that slot) left the earlier chip's scope window flat for the whole tune while the audience heard it play.
