...
`sid_panning.py`, config `[ultimate64].sid_panning`. U64, plus the Ultimate II+'s two emulated stereo SIDs via the same planners (`emusid1`/`emusid2` sources — see [`emusid_mixer.py`](#emusid_mixerpy--u2-emulated-stereo-sid-snoop-routing)).
...
`sid_volume.py`, config `[ultimate64].sid_volume`. U64, plus the Ultimate II+'s two emulated stereo SIDs via the same policy (see [`emusid_mixer.py`](#emusid_mixerpy--u2-emulated-stereo-sid-snoop-routing)).
...
## `emusid_mixer.py` — U2+ emulated stereo SID (snoop routing)
...
Deliberately out of scope for now: matching `SID Left/Right Filter Curve` / `Combined Waveforms` to the tune (the emu analog of [autoconfig](#sid-player-autoconfig) — the fields are read for the verdict but never written), enabling a disabled side, and multi-SID ASID streams on the U2+ (`AsidScene`'s `multi_sid` gate still requires the U64 surface; its routing call covers the single-chip case). `tests/test_emusid_mixer.py` covers the label round-trip, the topology read, the retarget planner, and the gated apply; the teardown ordering is pinned in `tests/test_waveform.py` and `tests/test_audio_source_sid.py`.
