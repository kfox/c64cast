...
**`0x31` at the scene: clamp, then dedupe.** `_apply_speed` derives the rate (frame delta wins, else the multiplier against the video rate) and hands it to `asid_player.clamp_frame_rate` — it does **not** re-implement the band; the constants and the warning text are single-sourced there, and `set_frame_rate` clamps again on the way in. The scene-side call exists so `_frame_rate_hz`, which the coalesced path and `setup`'s `start()` both read, agrees with the cadence the CIA was actually programmed for. Forwarding stays unconditional on arming (Symptom 2 above), but a request identical to the one in force returns early *before* the clamp: each retune is a blocking CIA-latch write plus a `flush()` round trip on the shared DMA socket — plus, pre-arm, a full handler re-upload — and an out-of-band request clamps with a WARNING, so a host repeating its `0x31` every frame would otherwise spend both the write budget and the log saying nothing new.
