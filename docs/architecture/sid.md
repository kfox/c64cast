...
**Re-activation re-derives the map.** Playlists reuse scene instances, so `setup` resets `_active_chips` / `_chip_addresses` / `_max_chip_seen` and the window count back to single-chip. Without that, lap 2 kept `_max_chip_seen` from lap 1, `process_frame`'s growth guard (`_max_chip_seen + 1 > _active_chips`) was false, `apply_sid_map` was never re-issued — and the scene went on writing chips 1..N to addresses the teardown restore had already un-mapped. The reset is at the top of `setup`, not in `teardown`, because teardown's silence loop still needs the mapped addresses.
