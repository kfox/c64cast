...
**The declared-host fallback (`[hardware].host_sid_model`).** A backend without the multi-SID config surface (TeensyROM has no config API; the Ultimate II+ has one without the SID categories) can't read any of this back, and originally the line was simply skipped there — which also skipped the single most audible verdict, the model match. Nothing on such a link can *ask* what chip the host C64 carries, but the user can declare it: `[hardware].host_sid_model` (`auto` | `6581` | `8580` | `unknown`) rides in on the backend profile (resolved by `make_backend`, next to the NTSC/PAL frame-rate resolution it already does), and `describe_declared_audio` renders the primary chip against it — `$D400 → host SID (6581 declared) — tune wants 8580` as a WARNING. Only the primary chip: a link that can't read SID state can't route extra chips either, so the host machine's own SID is all that plays. `auto` applies the NTSC=6581 / PAL=8580 convention, which is weak enough that the line says `assumed` rather than `declared` and the convention itself is logged once per run, explicitly as an assumption; `unknown` opts out of the verdict entirely. The fallback fires only when the backend *can't* read (`profile.supports_sid_config` false), not when a capable backend's read fails transiently — on a U64, "host SID" would be a wrong claim about a machine with four possible sources.
