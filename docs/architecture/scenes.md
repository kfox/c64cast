...
**Teardown steps are independent guarantees, not a transaction.** A scene's teardown owes the next scene a clean machine, and every promise in it — the display restored, the audio pump stopped, a capture handle released, a cartridge reset — stands on its own. Sequencing them in one `try` (or one bare statement list) means the first raise silently abandons the rest, and `Playlist.safe_teardown` swallows that raise, so the next scene starts on dirty hardware with nothing in the log to say why. `run_teardown_steps(log, who, steps)` takes `(label, callable)` pairs, guards each one, and logs `teardown step '<label>' failed; continuing` at ERROR. It catches `Exception` and not `BaseException` deliberately: teardown runs on the shutdown path, and a `KeyboardInterrupt` must propagate rather than be logged as a failed step and dropped. Every teardown in this module that owes more than one guarantee runs through it — `WebcamScene`, `SourceScene`, `VideoScene`, `LauncherScene` here, and the three SID scenes in [sid.md](sid.md#waveformpy--sidemupy--sid_host_emupy--sid-oscilloscope-scene), whose section also covers what their step *order* buys. Where a handle has to be dropped, the field is cleared before the call, so a `stop()` that raises does not leave a dead worker referenced.

Two boundaries on that rule, both deliberate. **A scene that owes one guarantee needs no runner**: `Scene.teardown` guards its own display-mode call, so `SlideshowScene`/`BlankScene`, whose only other statement is the trailing `audio.stop()`, have nothing that can starve anything — converting them would buy an untestable claim. And **the runner catches `Exception`, not `BaseException`**, which costs one case the `try/finally` in `SourceScene` used to cover: a `KeyboardInterrupt` inside a step now propagates instead of running the steps behind it. That is the right trade — teardown is the shutdown path, and an interrupt logged as a failed step and dropped would hang it — and it is narrow in practice, because playlists run on a worker thread, where `KeyboardInterrupt` is not delivered.
