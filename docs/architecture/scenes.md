...
`trails`, `pulse`, `rgb_shift`, `blur`. Applied in `scenes._render_with_overlays` before quantization, so every frame-bearing scene supports them.
...
**The `modulation is None` path is byte-stable**: `trails` uses its configured decay, and `pulse`/`rgb_shift` are the identity transform. Non-reactive scenes, the offline renderer, and the determinism tests are therefore unchanged. `pulse` and `rgb_shift` only visibly react on a `generative` + `audio_source = "sid"` scene, the only one with a feature stream today.
...
To add an effect: a `@register` subclass plus its name in `config._EFFECT_CHOICES` (drift-tested).
