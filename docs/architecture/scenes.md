...
**Teardown steps are independent guarantees, not a transaction.** A scene's teardown owes the next scene a clean machine, and every promise in it — the display restored, the audio pump stopped, a capture handle released, a cartridge reset — stands on its own. Sequencing them in one `try` (or one bare statement list) means the first raise silently abandons the rest, and `Playlist.safe_teardown` swallows that raise, so the next scene starts on dirty hardware with nothing in the log to say why. `run_teardown_steps(log, who, steps)` — in `c64cast/_teardown.py`, at the package root beside `_pollthread`, because the audio sources owe the same promises one layer down — takes `(label, callable)` pairs, guards each one, and logs `teardown step '<label>' failed; continuing` at ERROR. It catches `Exception` and not `BaseException` deliberately: teardown runs on the shutdown path, and a `KeyboardInterrupt` must propagate rather than be logged as a failed step and dropped. Every teardown in this module that owes more than one guarantee runs through it — `WebcamScene`, `SourceScene`, `VideoScene`, `LauncherScene` here, and the three SID scenes in [sid.md](sid.md#waveformpy--sidemupy--sid_host_emupy--sid-oscilloscope-scene), whose section also covers what their step *order* buys. So do the three audio sources a `SourceScene` composes — see [audio.md](audio.md#audio_sourcepy--audiofilesource-audio-file-reactive-source). Where a handle has to be dropped, the field is cleared before the call, so a `stop()` that raises does not leave a dead worker referenced. That holds where the teardown is the only reader of the field, and **not** where something else reads the handle's *liveness*: `PollThread.stop` deliberately keeps its thread reference when a bounded join times out, because a cleared one makes `is_running()` report False while the worker runs, and the next `start()` trusts that and clears the stop event the survivor is still reading. `AudioFileSource` is the one site here on the wrong side of that line — teardown drops the reference before a 2 s join that can time out, and `setup` then clears the stop event with no liveness check, so a survivor would be un-stopped while a second decode thread pushed into the same streamer. It predates the runner, and reaching it takes the 2 s join expiring with the decode thread still alive. `UltimateAudioSampler.push_samples` retries until the *sampler* stops rather than on a bound of its own, so once `start()` has raised there is no writer draining the queue and this source's `_stop` cannot release a decode thread parked on it: the 2 s join then expires with the thread still alive, which is the case [audio.md](audio.md#audio_sourcepy--audiofilesource-audio-file-reactive-source) describes. That route is backend-specific; a stalled read is not — `_decode_loop` tests `_stop` once per demuxed packet, and `resolve_file_spec` passes an `http(s)` media URL straight through to `av_open`, so a server that stops responding blocks inside `container.demux` where no check can see it, on the DAC path too. In an ordinary teardown the writer is still draining — `UltimateAudioSampler.stop` is the `audio stop` step *behind* the join, so it has not run yet — and the parked thread is released in milliseconds. (The 200 ms bound that would make the join look ample either way is `audio_handlers.QUEUE_PUT_TIMEOUT_S`, the 4-bit DAC path only.) Nor does a down link strand the survivor: `UltimateAudioSampler.stop` sets its stopped flag before any link I/O, so the `audio stop` step frees the thread whether or not its `gate_off` raises. The rule above still does not cover this and the fix belongs with the thread lifecycle rather than with the step guarding, so it is tracked separately rather than fixed here.
