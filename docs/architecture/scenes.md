...
**Guarding a step frees its position, but not always.** Because a failing step no longer starves the ones behind it, step order stops being a correctness property and becomes a readability one — with the exceptions noted here, each of which a tidy-up reorder would break silently. `AsidScene` closes its MIDI port before the restores, for the reason its [sid.md section](sid.md#waveformpy--sidemupy--sid_host_emupy--sid-oscilloscope-scene) gives. And **`VideoScene` logs its A/V lag summary before the audio stop**: `AudioStreamer.stop` clears its pushed-sample count and `UltimateAudioSampler.position_seconds` short-circuits once stopped, so a summary read afterwards divides a zero and reports `clock/wall=0.0000`. That gauge is the only clock/wall figure logged without `-v` — the live one is DEBUG — and `scripts/diags/mhires_tempo_clock_ab.py` parses it as `clock_final` when calibrating `[audio].dac_bitmap_tempo_hires/mhires`, so zeroing it would break a documented instrument rather than a log line. And **`AudioFileSource` stops its audio sink in front of its decode-thread join**, for the reason the paragraph above gives. And **`VideoScene` stops its audio in front of its source close**, which bounded-joins the demux thread that feeds `push_samples` — the same dependency one layer up, and the reason a sampler-audio clip used to spend the close's full 1 s bound. Each has its own guard: reversing the port close reddens `test_the_midi_port_closes_before_the_machine_is_restored` in `tests/test_asid_scene.py`, and in `tests/test_teardown_steps.py`, moving the summary behind the audio stop reddens `test_the_av_lag_summary_reads_the_clock_before_the_audio_stops`, putting the decode-thread join back in front of the audio stop reddens `test_file_audio_stops_the_sink_before_joining_decode`, and moving the source close back in front of it reddens `test_the_audio_stops_before_the_video_source_is_joined`. No test covers another's subject.
