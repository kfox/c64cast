...
**The self-served `GET /` control page** (`wled/wled_index.html`, read by `index_page_html()` — a dependency-free fetch-driven page for third-party WebView shell apps and any browser; it was a `_INDEX_HTML` triple-quoted string until it grew past 350 lines with no highlighting, formatter or linter reaching it, and its WebSocket client is now the one [`control/live_socket.js`](control.md#perf_consolepy--phone--web-performance-console-live-djvj-phase-5) shared with the `/perf` console rather than a second copy) mirrors exactly the functional controls: power, and per-system scene select, **palette select**, a per-segment **brightness slider**, speed/intensity sliders, and a **color picker**. The DOM is built client-side from `/json`, and each segment is titled by its `seg[].n` — the system name, with a single-system run using the configurable `[wled].name`, so it is never a bare "System 1".

The brightness slider earns its place because `bri` is a **real screen dim** (`_apply_dim` → `user_dim`, see the [`modes/`](video-color.md#modes--displaymode-hierarchy) note) and not merely an echo of power: `bri=0` dims all the way to black without pausing, and any nonzero value darkens the C64 output.
...
**And a cross-origin handshake is refused.** The bind gate above answers "who can route to this port"; it says nothing about *which page* in the operator's own browser can drive it, and on loopback that is the whole question. `POST /json` has checked `Origin` for a long time, but `/ws` — which applies the identical commands through `bridge.apply` — called `accept()` first and checked nothing. A WebSocket handshake is exempt from CORS, so unlike the POST there was never a preflight to fail: any page the operator visited mid-set could open `ws://<host>:8080/ws` and pause the run, jump scenes, sweep params or write presets, with a loopback bind making it *easier* rather than harder. `ws()` now refuses before `accept`, which is what makes uvicorn answer the handshake with an HTTP 403 instead of a close a browser reports as an ordinary disconnect (the same reasoning as `auth`'s module docstring and `perf_console.ConsoleFeed._refusal`).

`_is_cross_origin` delegates to `auth.same_origin` rather than deriving the comparison itself. It used to carry its own copy and the two had drifted: this module compared `netloc != host` with no guard on an empty `Host`, so a request with `Origin: null` and no `Host` read as same-origin here while `auth` refused it. One predicate, one behavior — and see `auth.same_origin`'s docstring for what neither copy stops (DNS rebinding).
