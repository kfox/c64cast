...
[c64cast/dac_calibration.py](../../c64cast/dac_calibration.py) owns the measurement run — bring-up, per-socket isolation, capture + retry — decomposed into module-level steps over a frozen `_RunContext` (`_bring_up_dac_env` → `_open_capture` → `_measure_one`/`_measure_each_socket` → always `_silence_and_reset` → `save_calibration` → `_report_run`), so the per-ring capture (`_capture_ring`) and per-SID measurement are plain functions a test can reach rather than closures.
[c64cast/dac_slot_ring.py](../../c64cast/dac_slot_ring.py) is the pure-numpy DSP: ring construction, extraction, every capture gate, and the sidtable fold — no hardware, no sounddevice, so the whole pipeline runs on synthetic captures in tests.
[c64cast/dac_capture_device.py](../../c64cast/dac_capture_device.py) picks and probes the sounddevice input.
[c64cast/dac_calibration_store.py](../../c64cast/dac_calibration_store.py) is persistence + identity keys.
[c64cast/dac_curve_resolve.py](../../c64cast/dac_curve_resolve.py) resolves `[audio].dac_curve` at playback.
