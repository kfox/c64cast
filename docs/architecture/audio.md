...
0.5% is 2.5× the healthy worst case. The failure carries its own type, `UnsteadyRingError`, because the advice inverts: every cause `_capture_fault_message` lists is about the recording not carrying the ring, and here it demonstrably does — so `_unsteady_ring_message` leads with "the input is right" and asks instead what else is reaching that output.

##### Which *kind* of unsteady, and what a run is worth

A spread number alone cannot say why the passes disagree, and the two causes have opposite fixes. `_pass_gain_decomposition` fits one scalar per pass — the level the whole ring came back at on that lap — and reports what survives it (`pass_residual_frac`, in the same units as `pass_spread_frac` so the two compare directly), alongside the per-pass gains themselves. A residual well under the spread means the ring replayed faithfully and only the level it was measured *through* was moving: re-measure once the path has settled. A residual close to the spread means the laps genuinely differ, and something else is reaching the output. On synthetic captures the two land far apart — a 2–20% drift or a settling envelope reads 0.01–0.22 of the spread, random per-block gain jitter reads 0.70–0.89 — so `PASS_RESIDUAL_DRIFT_RATIO` splits them at 0.5 with room on both sides. `_unsteady_ring_message` picks its cause list from that, because sending someone to disable a second SID cannot fix a capture whose level was still settling.

Measured on the rig that motivated it: a TR+ link into a C64U read 1.45% spread with a **1.41% residual** — the laps genuinely differed, so whatever it is, it is not a settling level. Beyond that the cause is **still unestablished**, and two plausible explanations were tested on hardware and refuted:

* **A second SID in the measurement.** That machine has two 6581s, but they sit at `$D400` and `$D420`, and auto-mirroring repeats in `$100` increments — so `$D418` writes never reach socket 2. Both UltiSID cores *are* mapped over the sockets, but their mixer volumes are `OFF`; they exist so the U64's LED lighting follows what the real SIDs are sent. Only socket 1 was ever audible at `$D400`. Isolating it by hand changed nothing audible, and the runs before and after differ by 1 completed run out of 2 against 0 out of 2 — chance, not a fix.
* **Other sources summed into the capture.** A real difference between the links, and worth knowing on its own account (see below), but not this fault: with socket 2 and both sampler channels muted by hand, three consecutive rings read 0.05%, 0.02%, 0.02% — the healthiest yet — and the next one still spiked to 1.25%.

That last pattern is the useful constraint. A steady additive contaminant would lift every ring roughly equally; it cannot leave three rings at 0.02% and spike the fourth. So the fault is an **intermittent event** during one capture window, and every continuous explanation is ruled out.

**Over a link with no config API, nothing else gets muted for you.** `_isolate_socket` and `_isolate_mixer` both run only inside the `supports_config` branch, so an Ultimate run measures with the target socket at unity and every other source — the second socket, both UltiSID cores, both sampler channels — forced to `OFF`, while a TR+ run on that same machine measures with all of them at whatever the user left them. On a two-SID machine that means a live 6581 idling into the captured output. Mute the other sources in the machine's own settings before calibrating over such a link; it is not what caused the fault above, but it is a real asymmetry between two runs that otherwise look identical.

That completed run is also why a per-ring note is not enough. Its rings sat at 0.2–0.44%, all under the trust gate, and the table it produced disagreed with the same chip measured cleanly over the Ultimate by **18% RMS (corr 0.84)** — against the **0.12% (corr 1.0000)** that two clean Ultimate runs of that chip twelve days apart reproduce at. Every ring passed, so nothing said anything. `_marginal_run_summary` now emits one line per run naming how many rings cleared `RING_SPREAD_HEALTHY` (0.2%) and the worst of them; the table is still written, because the gate's job is to refuse data it can prove is untrustworthy, not to guess a tighter number from one rig.

**Same-chip reproducibility is the yardstick, not table agreement.** Two clean measurements of one chip agree at corr 1.0000 / 0.12% RMS of span; two *different* chips measured just as cleanly agree at corr 0.76 and −0.15 on the same rig, and their 256-entry `sidtable`s match on 6 entries. So exact table agreement says almost nothing, and any claim that a measurement is good has to be made against a repeat of that same chip.

#### Keeping the evidence

A refused capture is the only evidence for its own refusal, and reproducing it costs a ~50 s hardware run that may not fault the same way — twice a calibration was rejected on a number nobody could go back and explain, once because the file was deleted and once because the waveform was simply dropped on the way out of `capture_ring`. `_save_unusable_capture` writes the refused waveform to `paths.unusable_capture_dir()` (`<data root>/calibration/unusable/<key>-<stamp>.npz`) with the codes, the sample rate and the diagnostics in the same file, since extraction needs all three to read it back; `np.load` plus one `extract_slot_levels` call reproduces the refusal offline. It is a diagnosis aid, so every failure in it is swallowed — it must never turn a measurement failure into a crash.
