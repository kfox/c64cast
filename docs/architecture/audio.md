...
Measured on the rig that motivated it: a TR+ link into a C64U with **two enabled 6581s, both UltiSID cores mapped at the same addresses, and auto-mirroring on** read 1.45% spread with a 1.41% residual — the laps genuinely differed, not a drift. Isolating socket 1 by hand over the Ultimate's config API let the same link complete a full 9-ring run; leaving it contaminated failed at ring 1 or 2 every time. That is the second-SID cause, isolated and reproduced. It is *not* a clean bill of health for the link: with the contamination removed the same link still failed intermittently (one run completed, the next failed at ring 2 with 1.28%), and the mechanism behind that remains unestablished.
