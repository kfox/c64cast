...
It is there because a finished table is otherwise undiagnosable. Both the sign inference and `lmax` derive from these numbers, and the failure modes are silent — a table whose signs were resolved badly looks exactly like a good one. With `raw_levels` a suspect calibration can be re-examined, and alternative ladder constructions trialled, entirely offline. Measured on two socketed 6581s: the two-reference model fits a chip whose filter path has degraded to ~1.4 % residual (it is degenerately unipolar, so there is nothing to resolve) but only ~25 % on a chip where Mahoney genuinely works, with the competing sign hypotheses within 10 % of each other — i.e. sign assignment approaches a coin flip on exactly the chips the feature exists to serve. Diagnosing that required the raw levels; the sidtable alone cannot show it.
