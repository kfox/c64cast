...
**What made that capture unsteady was never established, and the gate deliberately does not guess.** The two runs differed in the link they used *and* in whether the machine's second SID could be switched out of the way — `_isolate_socket` needs `supports_config`, so only the Ultimate run had it — and the file was deleted before those could be separated. It would be wrong to read this as "link X can't calibrate": the gate is a statement about the *data*, and a rig reading in the healthy band is unaffected however it connects. Calibrating over a link with no config API is a supported, useful thing to do, and the reported result of doing it is a considerable improvement; what it can't do is switch a second chip out of the measurement, which is worth doing by hand in the machine's own settings first.

0.5% is 2.5× the healthy worst case. The per-ring progress line marks anything above `RING_SPREAD_HEALTHY` (0.2%) as marginal, since a run whose rings all sit just under the gate is the one whose table is quietly poor. The failure carries its own type, `UnsteadyRingError`, because the advice inverts: every cause `_capture_fault_message` lists is about the recording not carrying the ring, and here it demonstrably does — so `_unsteady_ring_message` leads with "the input is right" and asks instead what else is reaching that output.
