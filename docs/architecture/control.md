...
**`TempoClock`.** Two drive modes behind one phase model:
...
Phase is `phase(now) = _phase_base + (now − _base_time)·bpm/60` while running. Each external pulse snaps `_phase_base` forward exactly and re-anchors `_base_time`; between pulses the extrapolation is **capped at one pulse's worth** (`1/PPQN`) so the phase can't run ahead of a late clock byte and then jerk backward when it lands. Internal/tap mode has no pulses, so the extrapolation *is* the advance, uncapped. `bar_phase` is `beat_phase / beats_per_bar` (accumulated bars — a launch quantizer watches `floor(bar_phase)` tick). All state lives behind a small `Lock` (the reads extrapolate several fields together); every method is cheap memory work, never DMA. `feed_message(msg)` is the mido bridge — duck-typed on `msg.type`/`msg.pos`, returns whether it consumed a clock message.
