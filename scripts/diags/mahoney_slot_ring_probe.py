...
re-run the whole extraction against those files with no hardware attached:
    ...
    scripts/diags/mahoney_slot_ring_probe.py --url u64://HOST --socket 1
    scripts/diags/mahoney_slot_ring_probe.py --replay scripts/diags/out/slotring

The hardware pass isolates one SID socket to $D400, brings up the NMI DAC +
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=OUT, help="where to save captures")
