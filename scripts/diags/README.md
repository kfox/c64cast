...
| [`video_render_probe.py`](video_render_probe.py) | Render a video through a display mode offline (no HW); reports per-frame bg0/$D021 flips, bitmap push churn, and **writes + modelled frame cost per region** (priced with the profile's measured link cost model) for flash/flicker and throughput diagnosis. Flags any region `write_region` split into more writes than pushing it whole would have cost. |
