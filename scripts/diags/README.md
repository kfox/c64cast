...
| [`flicker_score_grid.py`](flicker_score_grid.py) | Score which blend pairs actually flicker, by eye, blind. The eligibility rule behind `[color].flicker_max_warmth` is fitted to a session whose patches were laid out *sorted by ΔY*, so screen position was perfectly confounded with the variable; this replaces it. Positions are shuffled within a page and pages among themselves (seed printed, `--seed` replays it), pairs predicted to flicker never share a page with pairs predicted not to — a loud patch makes its neighbours unjudgeable — and every page hides one solid patch as a negative control, since a solid cannot flicker and calling one unsteady means the reading is picking up something else. Must be watched on the C64's own display, never a capture preview — a capture beats against the field-rate alternation and shows the picture slowly flipping between two versions of itself, which is completely convincing on a still page and makes scoring meaningless; the calibration page runs a capture-based check that the source really is alternating every field, so a slow flip after it passes is known to be downstream. A calibration page comes first with its three patches named — the loudest pair the clamp allows, a solid that cannot flicker, and a near-still blend — because otherwise the first scored page is judged against nothing and an all-cool page is indistinguishable from not knowing what to look for. Pools are dealt alternately so a run of warm pages cannot walk the scale. Which pair sat in which slot is revealed only after the last page. Self-paced: it waits on Enter, so **run it yourself**, not through an agent. Resets on exit. |
