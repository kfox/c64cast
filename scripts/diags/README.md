...
| [`vdc_preview.py`](vdc_preview.py) | **No hardware.** Show what an image becomes on the VDC in 640×200 8×2-colour bitmap mode — runs the real `c64cast.hw.vdc` packer + simulator, reports the colour-clash count (blocks with >2 colours), and can `--dump` the raw 24000-byte frame. Iterate on conversion quality offline. |
