...
| [`mahoney_slot_ring_probe.py`](mahoney_slot_ring_probe.py) | Drive the `--calibrate-dac` slot-ring primitive against one isolated SID socket and **save the raw captures to `.npy`**, so the alignment/extraction can be re-run offline with `--replay` and no hardware. Prints per-ring diagnostics, the merged 256-code ladder, and the volume-0 self-test. `--rounds N` sweeps how many slot-order rotations get averaged. Restores the SID config and resets on exit. |
