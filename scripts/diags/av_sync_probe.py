...
def main() -> int:
    ...
    # --gen-source posture knobs (written into the auto-config) so one tool drives
    # the A/B: pure resample = --max-hz <sample_rate>; HW-safe = --max-hz <mid>;
    # max-bandwidth = --max-hz 0 (default); baseline = --no-resample --no-adaptive.
    ap.add_argument("--max-hz", type=int, default=0, help="gen-source [audio].nmi_rate_max_hz")
    ap.add_argument("--no-resample", action="store_true", help="gen-source: disable resample")
    ap.add_argument("--no-adaptive", action="store_true", help="gen-source: disable adaptive loop")
...
def _write_gen_config(out_dir: Path, mp4: Path, args) -> Path:
    ...
    with audio on, at the posture under test (the --max-hz / --no-resample /
    --no-adaptive knobs select pure-resample / HW-safe / max-bandwidth / baseline)."""
...
nmi_rate_adaptive = {"false" if args.no_adaptive else "true"}
nmi_rate_resample = {"false" if args.no_resample else "true"}
nmi_rate_max_hz = {args.max_hz}
