...
def main() -> int:
    ...
    ap.add_argument(
        "--use-reu-pump",
        action="store_true",
        help="gen-source: route audio through the REU pump (no host-DMA audio "
        "writes) instead of the host-DMA worker. A/B the bed's modulation spectrum "
        "vs host-DMA to compare the write-cadence wobble against the REU-path AM.",
    )
    ap.add_argument(
        "--target-fps",
        type=int,
...
def _write_gen_config(out_dir: Path, mp4: Path, args) -> Path:
    ...
    reu_line = "use_reu_pump = true" if args.use_reu_pump else ""
    cfg.write_text(
