...
| Phone/web performance console | off; shares the control-plane server | Rides the same port, so reaching it from a phone means binding the control plane to a LAN address — which exposes the control plane too, under the same token or the same absence of one. Its two write surfaces (`POST /perf/command` and the `/perf/ws` command frames) refuse a request whose `Origin` names a different host:port than the one it reached, and the POST requires `application/json` — a WebSocket handshake is outside CORS entirely and `Request.json()` ignores the content type, so without that a page the performer merely *visited* could drive the show in the unauthenticated mode. A request with no `Origin` at all (a script, `curl`, `wscat`) is still served: that is the caller the open mode is for. The command body is capped, and the number of open state sockets is capped. |
