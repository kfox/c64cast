...
| Web console media picker (`[web].media_roots`) | the four default asset directories | Read-only listing of media a `file =` field could name, confined to the configured roots the same way the config browser is. No write path exists — nothing is ever created, replaced or deleted here. A `viewer` token may browse it, same as it may watch the screen. |
